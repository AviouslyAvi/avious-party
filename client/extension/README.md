...
3. Manifest needs `content_scripts` matching `<all_urls>` (or whichever sites we're scoping to) and `all_frames: true` so the script runs in the player iframe too.
